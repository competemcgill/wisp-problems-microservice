...
describe("Problems controller tests", () => {
    ...
    describe("GET /users", () => {
        it("status 200: returns successfully a list of 1 problem", async () => {
            const { body: problems } = await chai.request(app).get("/problems");
            expect(problems.length).to.equal(1);
            expect(problems[0].problemId).to.equal(testProblem.problemId);
        });
    });
});